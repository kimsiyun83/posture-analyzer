...
import {readInbodyBasics,readInbodyDate,visualRows} from '@/lib/inbody-ocr';
...
export default function InbodyImport({onSaved,onCancel}:{onSaved:()=>void;onCancel:()=>void}){
 const [date,setDate]=useState(''),[image,setImage]=useState(''),[values,setValues]=useState<Record<string,string>>({}),[busy,setBusy]=useState(false),[error,setError]=useState(''),[consent,setConsent]=useState(false),[login,setLogin]=useState(false),[progress,setProgress]=useState(''),[recognizing,setRecognizing]=useState(false),[raw,setRaw]=useState(''),[dateNotice,setDateNotice]=useState('결과지의 검사일을 확인해 주세요.');
 ...
 function stopRecognition(){cancelled.current=true;cancelPending.current?.();void worker.current?.terminate().catch(()=>{});setDateNotice('결과지에 표시된 검사일을 확인해 주세요.');setProgress('자동 인식을 중단했습니다. 사진은 유지되며 수치를 직접 입력할 수 있습니다.');}
 async function recognize(canvas:HTMLCanvasElement){
  ...
  try{
   ...
   timer=setTimeout(()=>{stopped=true;cancelPending.current?.();void worker.current?.terminate().catch(()=>{});if(alive.current){setDateNotice('검사일을 직접 입력해 주세요.');setProgress('인식 시간이 초과되었습니다. 다시 인식하거나 수치를 직접 입력해 주세요.');}},90000);
   ...
   const first=await waitFor(w.recognize(canvas,{}, {text:true,tsv:true}));const texts=[first.data.text,visualRows(first.data.tsv??'')];const passes=[readInbodyBasics(first.data.text,first.data.tsv??'')];
   if(Object.keys(passes[0]).length<4||!readInbodyDate(first.data.text)){
    await waitFor(w.setParameters({tessedit_pageseg_mode:PSM.SPARSE_TEXT}));
    for(const [index,left,width] of [[1,0,1],[2,0,.67],[3,.64,.36]]){
     if(!alive.current||stopped)return;stage=`상세 영역 ${index}/3`;
     const r=await waitFor(w.recognize(canvas,{rectangle:{left:Math.floor(canvas.width*left),top:0,width:Math.floor(canvas.width*width),height:canvas.height}},{text:true,tsv:true}));passes.push(readInbodyBasics(r.data.text,r.data.tsv??''));texts.push(r.data.text,visualRows(r.data.tsv??''));
    }
   }
   if(!alive.current||stopped)return;
   const found=mergeReportReadings(passes);const entries=fields.flatMap(([k])=>found[k]?[[k,String(found[k]!.value)]]:[]);
   const dates=[...new Set(texts.map(readInbodyDate).filter((d):d is string=>!!d))];
   setRaw(texts.join('\n\n---\n'));if(dates.length===1){setDate(dates[0]);setDateNotice('검사일을 자동 입력했습니다. 결과지와 비교해 주세요.');}else{setDate('');setDateNotice('검사일을 확정하지 못했습니다. 결과지에 표시된 날짜를 직접 입력해 주세요.');}
  ...
  }catch{if(alive.current&&!stopped){setDateNotice('검사일을 직접 입력해 주세요.');setProgress('자동 인식에 실패했습니다. 사진은 유지됩니다. 다시 인식하거나 수치를 직접 입력해 주세요.');}}
 ...
 async function photo(file:File){if(operation.current)return;operation.current=true;setBusy(true);setError('');try{
  ...
  if(alive.current){setImage(data);setValues({});setDate('');setRaw('');setDateNotice('검사일을 인식하고 있습니다.');changed();ocrCanvas.current=canvas;await recognize(canvas);}
 ...
 return <section className="ib-import"><h1>검사 기록 추가</h1><p>결과지 사진은 검사일별로 누적 보관합니다. 사진에서 검사일과 체중·골격근량·체지방량·체지방률을 찾아 자동 입력합니다. 수치 없이 사진만 저장해도 됩니다.</p>
 ...
 {raw&&<details><summary>자동 인식 원문 확인</summary><pre style={{whiteSpace:'pre-wrap',maxHeight:240,overflow:'auto'}}>{raw}</pre></details>}<label className="check-label"><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/>검사일과 입력값을 확인했으며 결과지 사진·수치를 내 계정에 저장하고 센터 관리자가 조회하는 데 동의합니다.</label></fieldset>
 <p role="alert">{error}</p>{login&&<CustomerLogin onLogin={()=>{setLogin(false);void save();}}/>}<button className="care-primary" disabled={busy||!consent||!date} onClick={()=>void save()}>{busy?'처리 중…':'검사 기록 저장'}</button><button className="care-secondary" disabled={busy} onClick={onCancel}>취소</button></section>;
}

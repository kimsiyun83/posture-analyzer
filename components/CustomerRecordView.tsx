...
export default function CustomerRecordView({kind,data,date}:{kind:string;data:unknown;date:string}){
 if(kind==="inbody"){const r=parseReport(data,!!(data as Record<string,unknown>)?.hasImage);return r?<><p>{r.archived?"보관함 · ":""}{new Date(r.measuredAt).toLocaleString("ko-KR")}</p><InbodyReportView report={r} hasImage={!!(data as Record<string,unknown>).hasImage} sourceLink="/inbody"/></>:<p>인바디 기록을 읽을 수 없습니다.</p>;}

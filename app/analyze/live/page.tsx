...
function Live(){
 ...
 useEffect(()=>{
  ...
  async function start(){try{
   ...
   function tick(now:number){if(cancelled)return;frame=requestAnimationFrame(tick);if(now-last<50||v.readyState<2||v.currentTime===lastVideo)return;last=now;lastVideo=v.currentTime;
    try{
     const out=model!.detectForVideo(v,now),p=out.landmarks.length===1?out.landmarks[0]:[];
     const left=side==="left",indices=kind==="balance"?[11,12,23,24,27,28]:kind==="elbow"?(left?[11,13,15]:[12,14,16]):(left?[23,11,13]:[24,12,14]);
 ...
 return <div className="care-app live-assessment"><header className="care-header"><Link href="/analyze/tests">← 검사 목록</Link><b>실시간 카메라 검사</b></header><main className="care-main"><h1>{balance?"한발서기 자동 측정":kind==="elbow"?"팔꿈치 굽힘 각도":"어깨 올림 각도"}</h1><section className="care-card"><p>{balance?"정면에서 전신과 바닥이 보이게 촬영하세요. 측정할 지지발을 선택하고 양발로 선 뒤 반대쪽 발을 들어 주세요. 최대 180초까지 측정합니다.":"측정할 팔이 카메라 쪽을 향하도록 서세요. 어깨 올림은 몸통–어깨–팔꿈치, 팔꿈치 굽힘은 어깨–팔꿈치–손목을 기준으로 측정합니다."}</p><p className="muted small">카메라 영상의 2차원 참고 수치입니다. 실제 관절 가동범위 진단을 대신하지 않습니다. {balance?"손으로 벽을 짚는 동작은 자동 판별하지 못합니다. 직원과 함께 안전한 공간에서 검사하세요.":"카메라와 팔의 방향을 맞추고 몸통을 움직이지 마세요."} 영상은 서버에 저장하지 않습니다.</p></section>{!balance&&<details className="care-card"><summary>측정 자세 예시 보기</summary><JointMeasurementGuide kind={kind}/></details>}<div className="goal-options">{(["left","right"] as const).map(s=><button key={s} disabled={active} aria-pressed={side===s} onClick={()=>{setSide(s);setValue(null);setPeak(null);measured.current=null;setComplete(null);}}>{s==="left"?"왼쪽":"오른쪽"} {balance?"지지발":"팔"}</button>)}</div><button type="button" className="care-secondary" aria-pressed={mirror} onClick={()=>setMirror(m=>!m)}>좌우 반전 · 거울 모드 {mirror?"켜짐":"꺼짐"}</button><div ref={cameraBox} className={`live-camera${mirror?" live-camera-mirrored":""}`}><video ref={video} playsInline muted style={{transform:mirror?"scaleX(-1)":"none"}}/><canvas ref={canvas} style={{transform:mirror?"scaleX(-1)":"none"}}/><div className="live-camera-status" role="status">{message}</div><div className="live-reading">{value===null?"—":balance?value.toFixed(1):value}{balance?"초":"°"}</div></div>{!balance&&<p>이번 최대 각도: {peak??"—"}°</p>}<button className="care-primary" disabled={!enabled} onClick={()=>{setComplete(null);setValue(null);setPeak(null);setMessage("카메라와 관절 인식을 준비하고 있습니다…");setActive(a=>!a);setAttempt(a=>a+1);}}>{active?"측정 취소 · 카메라 끄기":"카메라 켜고 측정 시작"}</button>{!balance&&<button className="care-secondary" disabled={!active||peak===null} onClick={()=>{if(measured.current!==null){setResults(r=>({...r,[side]:measured.current!}));setActive(false);setMessage("각도를 기록했습니다. 반대쪽도 측정해 주세요.");}}}>이번 최대 각도 기록</button>}<section className="care-card"><h2>좌우 검사 결과</h2><p>왼쪽 {results.left??"미측정"} / 오른쪽 {results.right??"미측정"} {balance?"초":"°"}</p><button className="care-primary" disabled={active||results.left===undefined||results.right===undefined||!!complete} onClick={()=>setComplete({id:crypto.randomUUID(),data:{left:results.left!,right:results.right!,source:"camera"}})}>검사 완료 · 계정에 저장</button></section>{complete&&<CustomerSave kind={kind} data={complete.data} clientId={complete.id}/>}</main></div>;
}
